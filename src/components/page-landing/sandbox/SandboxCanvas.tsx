...
const STICKER_CONFIG: StickerConfig[] = [
  { file: "002.png", x: 0.5, y: 0.5, angle: 0 },
  ...
  { file: "007.png", x: 0.2796, y: 0.729, angle: -0.15 },
  ...
  { file: "012.png", x: 0.798, y: 0.5613, angle: -0.07 },
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const availableFilenames = new Set(
        availablePaths.map((p) => p.split("/").pop()!),
      );
      const pathMap = Object.fromEntries(
        availablePaths.map((p) => [p.split("/").pop()!, p]),
      );
      const validConfig = STICKER_CONFIG.filter((cfg) =>
        availableFilenames.has(cfg.file),
      );
      ...
      if (validConfig.length === 0) {
        ...
        ctx.fillText(
          "No matching stickers found in /public/stickers/",
          W / 2,
          H / 2,
        );
        return;
